export default {
  ...
  /**
   * Determine the luminance (basically brightness) of the RGB
   * array provided by converting it to HSV/HSL
   *
   * @param {Array} rgb
   * @param {Number} maxOfRange Guarantees only integers between 0 and maxOfRange
   * @returns {number}
   */
  getRgbLuminance: function (rgb, maxOfRange) {

    var hsv = this.rgbToHsv(rgb);

    return parseInt(hsv[2] * maxOfRange, 10) / maxOfRange;
  ...
  rgbToHsv: function (rgb) {
    ...
    if (!(this.cache && this.cache[r] && this.cache[r][g] && this.cache[r][g][b])) {

      this.cache = this.cache || {};
      this.cache[r] = this.cache[r] || {};
      this.cache[r][g] = this.cache[r][g] || {};
      this.cache[r][g][b] = this._rgbToHsv(r, g, b);

    }

    return this.cache[r][g][b];
